# PlanktoQNT

<!-- <img src="docs/img/logo.webp" align="right" height="100"/> -->

[![License: MIT](https://img.shields.io/badge/license-MIT-green.svg)](https://opensource.org/licenses/MIT) [![Python](https://img.shields.io/badge/python-3.12.9-3776AB?logo=python&logoColor=white)](https://www.python.org/)

**Currently under development**

PlanktoQNT is a modular Python pipeline for quantitative analysis of plankton images from the Imaging FlowCytobot (IFCB) data after classification with [PlanktoNET](https://github.com/ECBSU/PlanktoNET). PlanktoQNT transforms classified regions of interest (ROIs) into ecological metrics, including cell abundance, size structure, biovolume, and carbon biomass. PlanktoQNT provides reproducible outputs for statistical analysis, ecosystem modeling, and carbon cycling studies of aquatic systems.

![workflow](docs/img/workflow.webp)

The classified images should be stored in different folders, according to species/group.

The latest PlanktoNET model (ver1_6) can classify 86 categories from the following groups:

```
Categories/
├── Diatoms/
│   ├── Centric_diatoms/
│   │   ├── Bacillaria
│   │   ├── Biddulphia
│   │   ├── Cerataulina
│   │   ├── Chaetoceros
│   │   ├── Leptocylindrus
│   │   ├── Rhizosolenia
│   │   ├── Skeletonema
│   │   └── Diatom_centrales
│   ├── Pennate_diatoms/
│   │   ├── Cylindrotheca
│   │   ├── Cylindrotheca_big
│   │   ├── Diploneis
│   │   ├── Grammatophora
│   │   ├── Licmophora
│   │   ├── Navicula
│   │   ├── Pleurosigma
│   │   ├── Pseudonitzschia
│   │   ├── Tabellaria
│   │   └── Diatom_pennate
│
├── Dinoflagellates/
│   ├── Thecate_dinoflagellates/
│   │   ├── Alexandrium_sp
│   │   ├── Alexandrium_sp_big
│   │   ├── Alexandrium_sp_empty
│   │   ├── Alexandrium_tamarense
│   │   ├── Blixaea_quinquecornis
│   │   ├── Blixaea_quinquecornis_empty
│   │   ├── Ceratium_furca
│   │   ├── Ceratium_fusus
│   │   ├── Ceratium_sp1
│   │   ├── Ceratium_sp2
│   │   ├── Gambierdiscus
│   │   ├── Gambierdiscus_empty
│   │   ├── Gonyaulax
│   │   ├── Gonyaulax_empty
│   │   ├── Heterocapsa_lanceolata
│   │   ├── Heterocapsa_rotundata
│   │   ├── Heterocapsa_sp
│   │   ├── Heterocapsa_triquetra
│   │   ├── Ostreopsidaceae
│   │   ├── Prorocentrum_concavum
│   │   ├── Prorocentrum_lima
│   │   ├── Prorocentrum_lima_empty
│   │   ├── Prorocentrum_minimum
│   │   ├── Prorocentrum_sp1
│   │   ├── Protoperidinium_bipes
│   │   ├── Protoperidinium_bipes_empty
│   │   ├── Protoperidinium_brevipes
│   │   ├── Protoperidinium_ovatum
│   │   ├── Protoperidinium_pallidum
│   │   ├── Protoperidinium_steinii
│   │   ├── Scrippsiella_trochoidea
│   │   └── Scrippsiella_trochoidea_empty
│   ├── Athecate_dinoflagellates/
│   │   ├── Akashiwo_sanguinea
│   │   ├── Cochlodinium
│   │   ├── Gymnodinium_big
│   │   ├── Gymnodinium_sp
│   │   ├── Karlodinium_sp
│   │   ├── Symbiodiniaceae
│   │   └── Dinoflagellate_cluster
│
├── Ciliates/
│   ├── Euplotes_sp
│   ├── Mesodinium_sp
│   ├── Strobilidium
│   ├── Strombidium_capitatum
│   ├── Tintinids
│   └── Tintinnid_house
│
├── Cyanobacteria/
│   ├── Cyanobacteria_filam
│   ├── Trichodesmium
│   └── Moorena
│
├── Other_phytoplankton_and_flagellates/
│   ├── Coccolith-like
│   ├── Cryptomonas
│   ├── Dinobryon
│   ├── Euglenophyceae
│   ├── Heterosigma_akashiwo
│   ├── Nanoflagellate_cluster
│   └── Nanoflagellates_other
│
├── Zooplankton_and_protists/
│   ├── Foraminifera
│   ├── Nauplii
│   ├── Radiolaria
│   └── Astramoeba
│
└── Detritus_and_artifacts/
    ├── Detritus
    ├── Detritus_clear
    ├── Detritus_small
    ├── Fecal_pellet
    ├── Exoskeleton_parts
    ├── Bubble
    ├── White_stick
    ├── Elongated_empty
    └── Bad
```
